...
function findWordToUse(){
 ...
 var throwword=retireHand();
	if(throwword){
		var throwzm=0;
		throwzm+=7;	
		}
	var Usword=$( "#human-word-input").length;
	Usword+=Usword; 
 	
	if(BAG_OF_LETTERS.length-throwzm-Usword<=0)
	{
    var maxnum = 0;
	var maxzm = "";
	...
	alert("结束游戏  最高分数为："+maxnum + " 单词为："+maxzm);
	}else{
		startGame();
		}	
}
